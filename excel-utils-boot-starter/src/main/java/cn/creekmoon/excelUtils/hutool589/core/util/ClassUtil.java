...
import cn.creekmoon.excelUtils.hutool589.core.convert.BasicType;
...
import cn.creekmoon.excelUtils.hutool589.core.lang.ClassScanner;
import cn.creekmoon.excelUtils.hutool589.core.lang.Filter;
import cn.creekmoon.excelUtils.hutool589.core.lang.Singleton;
...
public class ClassUtil {
	...
	/**
	 * 获取完整类名的短格式如：<br>
	 * cn.creekmoon.excelUtils.hutool589.core.util.StrUtil -》c.h.c.u.StrUtil
	 *
	 * @param className 类名
	 * @return 短格式类名
	 * @since 4.1.9
	...
	public static String getShortClassName(String className) {
		final List<String> packages = cn.creekmoon.excelUtils.hutool589.core.util.StrUtil.split(className, cn.creekmoon.excelUtils.hutool589.core.util.CharUtil.DOT);
		if (null == packages || packages.size() < 2) {
			return className;
		}

		final int size = packages.size();
		final StringBuilder result = cn.creekmoon.excelUtils.hutool589.core.util.StrUtil.builder();
		result.append(packages.get(0).charAt(0));
		for (int i = 1; i < size - 1; i++) {
			result.append(cn.creekmoon.excelUtils.hutool589.core.util.CharUtil.DOT).append(packages.get(i).charAt(0));
		}
	...
	public static boolean equals(Class<?> clazz, String className, boolean ignoreCase) {
		if (null == clazz || cn.creekmoon.excelUtils.hutool589.core.util.StrUtil.isBlank(className)) {
			return false;
		}
		if (ignoreCase) {
	...
	public static Field getDeclaredField(Class<?> clazz, String fieldName) throws SecurityException {
		if (null == clazz || cn.creekmoon.excelUtils.hutool589.core.util.StrUtil.isBlank(fieldName)) {
			return null;
		}
		try {
	...
	public static Set<String> getClassPathResources(boolean isDecode) {
		return getClassPaths(cn.creekmoon.excelUtils.hutool589.core.util.StrUtil.EMPTY, isDecode);
	}
	...
	public static Set<String> getClassPaths(String packageName, boolean isDecode) {
		String packagePath = packageName.replace(cn.creekmoon.excelUtils.hutool589.core.util.StrUtil.DOT, cn.creekmoon.excelUtils.hutool589.core.util.StrUtil.SLASH);
		...
		while (resources.hasMoreElements()) {
			...
			paths.add(isDecode ? cn.creekmoon.excelUtils.hutool589.core.util.URLUtil.decode(path, CharsetUtil.systemCharsetName()) : path);
		}
		return paths;
	}
	...
	public static URL getClassPathURL() {
		return getResourceURL(cn.creekmoon.excelUtils.hutool589.core.util.StrUtil.EMPTY);
	}
	...
	public static boolean isAllAssignableFrom(Class<?>[] types1, Class<?>[] types2) {
		if (cn.creekmoon.excelUtils.hutool589.core.util.ArrayUtil.isEmpty(types1) && ArrayUtil.isEmpty(types2)) {
			return true;
		}
	...
	public static <T> T invoke(String classNameWithMethodName, boolean isSingleton, Object... args) {
		if (cn.creekmoon.excelUtils.hutool589.core.util.StrUtil.isBlank(classNameWithMethodName)) {
	...
	public static <T> T invoke(String className, String methodName, boolean isSingleton, Object... args) {
		...
		try {
			...
			if (null == method) {
				throw new NoSuchMethodException(cn.creekmoon.excelUtils.hutool589.core.util.StrUtil.format("No such method: [{}]", methodName));
			}
			if (isStatic(method)) {
	...
	public static Class<?> getTypeArgument(Class<?> clazz, int index) {
		final Type argumentType = cn.creekmoon.excelUtils.hutool589.core.util.TypeUtil.getTypeArgument(clazz, index);
	...
	public static String getPackage(Class<?> clazz) {
		if (clazz == null) {
			return cn.creekmoon.excelUtils.hutool589.core.util.StrUtil.EMPTY;
		}
		final String className = clazz.getName();
		int packageEndIndex = className.lastIndexOf(cn.creekmoon.excelUtils.hutool589.core.util.StrUtil.DOT);
	...
	public static String getPackagePath(Class<?> clazz) {
		return getPackage(clazz).replace(cn.creekmoon.excelUtils.hutool589.core.util.StrUtil.C_DOT, StrUtil.C_SLASH);
	}
