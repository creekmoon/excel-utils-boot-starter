...
public class CreditCodeUtil {
	...
	public static String randomCreditCode() {
		...
		//
		for (int i = 0; i < 2; i++) {
			int num = cn.creekmoon.excelUtils.hutool589.core.util.RandomUtil.randomInt(BASE_CODE_ARRAY.length - 1);
			buf.append(Character.toUpperCase(BASE_CODE_ARRAY[num]));
		}
		for (int i = 2; i < 8; i++) {
			int num = cn.creekmoon.excelUtils.hutool589.core.util.RandomUtil.randomInt(10);
