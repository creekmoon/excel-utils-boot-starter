...
import cn.creekmoon.excelUtils.hutool589.core.net.URLDecoder;
import cn.creekmoon.excelUtils.hutool589.core.net.URLEncodeUtil;
import cn.creekmoon.excelUtils.hutool589.core.net.url.UrlQuery;
...
public class URLUtil extends URLEncodeUtil {
	...
	public static URI getStringURI(CharSequence content) {
		...
		final String contentStr = cn.creekmoon.excelUtils.hutool589.core.util.StrUtil.addPrefixIfNot(content, "string:///");
	...
	public static String completeUrl(String baseUrl, String relativePath) {
		...
		if (cn.creekmoon.excelUtils.hutool589.core.util.StrUtil.isBlank(baseUrl)) {
			return null;
		}
	...
	public static String decode(String content, String charset) throws UtilException {
		return decode(content, cn.creekmoon.excelUtils.hutool589.core.util.StrUtil.isEmpty(charset) ? null : CharsetUtil.charset(charset));
	}
	...
	public static URI toURI(String location, boolean isEncode) throws UtilException {
		...
		try {
			return new URI(cn.creekmoon.excelUtils.hutool589.core.util.StrUtil.trim(location));
		} catch (URISyntaxException e) {
			throw new UtilException(e);
		}
	}
	...
	public static String normalize(String url, boolean isEncodePath, boolean replaceSlash) {
		if (cn.creekmoon.excelUtils.hutool589.core.util.StrUtil.isBlank(url)) {
			return url;
		}
		...
		String body;
		if (sepIndex > 0) {
			protocol = cn.creekmoon.excelUtils.hutool589.core.util.StrUtil.subPre(url, sepIndex + 3);
			body = cn.creekmoon.excelUtils.hutool589.core.util.StrUtil.subSuf(url, sepIndex + 3);
		} else {
			protocol = "http://";
			body = url;
		}

		final int paramsSepIndex = cn.creekmoon.excelUtils.hutool589.core.util.StrUtil.indexOf(body, '?');
		String params = null;
		if (paramsSepIndex > 0) {
			params = cn.creekmoon.excelUtils.hutool589.core.util.StrUtil.subSuf(body, paramsSepIndex);
			body = cn.creekmoon.excelUtils.hutool589.core.util.StrUtil.subPre(body, paramsSepIndex);
		}

		if (cn.creekmoon.excelUtils.hutool589.core.util.StrUtil.isNotEmpty(body)) {
			// 去除开头的\或者/
			//noinspection ConstantConditions
			body = body.replaceAll("^[\\\\/]+", cn.creekmoon.excelUtils.hutool589.core.util.StrUtil.EMPTY);
			// 替换\为/
			body = body.replace("\\", "/");
			if (replaceSlash) {
		...
		final int pathSepIndex = cn.creekmoon.excelUtils.hutool589.core.util.StrUtil.indexOf(body, '/');
		String domain = body;
		String path = null;
		if (pathSepIndex > 0) {
			domain = cn.creekmoon.excelUtils.hutool589.core.util.StrUtil.subPre(body, pathSepIndex);
			path = cn.creekmoon.excelUtils.hutool589.core.util.StrUtil.subSuf(body, pathSepIndex);
		}
		if (isEncodePath) {
			path = encode(path);
		}
		return protocol + domain + cn.creekmoon.excelUtils.hutool589.core.util.StrUtil.nullToEmpty(path) + cn.creekmoon.excelUtils.hutool589.core.util.StrUtil.nullToEmpty(params);
	}
	...
	public static String getDataUri(String mimeType, Charset charset, String encoding, String data) {
		final StringBuilder builder = cn.creekmoon.excelUtils.hutool589.core.util.StrUtil.builder("data:");
		if (cn.creekmoon.excelUtils.hutool589.core.util.StrUtil.isNotBlank(mimeType)) {
