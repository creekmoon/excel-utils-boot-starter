...
import cn.creekmoon.excelUtils.hutool589.core.clone.CloneSupport;
...
public class DynaBean extends CloneSupport<DynaBean> implements Serializable {
	...
	public <T> T get(String fieldName) throws BeanException {
		if (Map.class.isAssignableFrom(beanClass)) {
			...
			final PropDesc prop = cn.creekmoon.excelUtils.hutool589.core.bean.BeanUtil.getBeanDesc(beanClass).getProp(fieldName);
			if (null == prop) {
	...
	public boolean containsProp(String fieldName) {
		if (Map.class.isAssignableFrom(beanClass)) {
			...
			return null != cn.creekmoon.excelUtils.hutool589.core.bean.BeanUtil.getBeanDesc(beanClass).getProp(fieldName);
		}
	}
