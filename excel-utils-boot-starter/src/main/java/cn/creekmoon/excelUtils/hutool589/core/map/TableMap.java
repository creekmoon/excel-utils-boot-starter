...
import cn.creekmoon.excelUtils.hutool589.core.util.ObjUtil;
...
public class TableMap<K, V> implements Map<K, V>, Iterable<Map.Entry<K, V>>, Serializable {
	...
	public Set<Entry<K, V>> entrySet() {
		...
		for (int i = 0; i < size(); i++) {
			hashSet.add(cn.creekmoon.excelUtils.hutool589.core.map.MapUtil.entry(keys.get(i), values.get(i)));
		}
		return hashSet;
	}
